...
## What this script no longer touches

It used to rewrite a pinned-repository card between `<!-- pin:start -->` and
`<!-- pin:end -->`, from a repository name hard-coded in the script. Those
markers are gone. The project cards now live inside `<!-- projects:start -->`
and are regenerated by `scripts/update-profile.py`, which picks the three
most recently worked-on public repositories on its own.
